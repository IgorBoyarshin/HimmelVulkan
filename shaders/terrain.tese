...
void main() {
    ...
    float unit = abs(length(inTexCoord[0] - inTexCoord[1])) / inSegments[0] / 2.0; // NOTE last division is imperical
    ...
    vec3 normal = normalize(vec3(hL - hR, 2.0 * unit, hD - hU));

    vec4 cameraPosition = ubo.view * vec4(v0);
